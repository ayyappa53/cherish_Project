...
const searchCars = async (req, res) => {
  ...
  try {
    ...
    const cars = await Car.find({
      $or: [
        { title: { $regex: q, $options: "i" } }, // Case-insensitive partial match on title
        { tags: { $in: [q] } } // Exact match on tags
      ]
    });

    if (cars.length === 0) {
      return res.status(404).json({ success: false, message: "No matching cars found" });
    }
...
module.exports = { registerUser, loginUser, getUserData, getRefreshToken, searchCars};
